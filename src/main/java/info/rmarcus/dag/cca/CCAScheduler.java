...
public class CCAScheduler {
  public static Collection<TaskQueue> cca(Collection<Task> tasks, int deadline) {
    ...
    for (TaskQueue tq : toR) {
      priority.put(
          tq,
          computePriority(tq.getTasks(), deadline)
              .values()
              .stream()
              .mapToInt(i -> i)
              .max()
  ...
  private static int communication(Task t1, Task t2) {
    if (t1.getTaskQueue() == t2.getTaskQueue()) return 0;

    if (!t1.getDependents().containsKey(t2)) return 0;

    return t1.getDependents().get(t2);
  }

  private static Set<Task> getSources(Collection<Task> tasks) {
    return tasks.stream().filter(t -> t.getDependencies().size() == 0).collect(Collectors.toSet());
  }

  private static Set<Task> getSinks(Collection<Task> tasks) {
    return tasks.stream().filter(t -> t.getDependents().size() == 0).collect(Collectors.toSet());
  }
}
