...
public class PermutationSolver {
  ...
  private static int evaluateRemovalOfPart(
      List<Task> t, Set<Integer> currentPartitions, int toRemove) {
    ...
    if (actualized == null) return -1;

    int costWithout = CostAnalyzer.getLatency(actualized);
    return costWithout;
  }

  private static int evaluateRemovalOfRange(
      List<Task> t, Set<Integer> current, Set<Integer> toConsider) {
  ...
  private static int findBestPartitionGreedy(
      List<Task> t, Set<Integer> currentPartitions, ExecutorService exec) {
  ...
  private static List<TaskQueue> buildQueuesWithPartitions(
      List<Task> tasks, Set<Integer> partitions) {
    ...
    for (Task t : tasks) {
      ...
      if (partitions.contains(cnt)) tq.add(new TaskQueue(MachineType.SMALL));
      cnt++;
    }
    return tq;
  }
  ...
  public static List<TaskQueue> greedySolve(List<Task> tasks) {
    ...
    for (int i = 0; i < tasks.size() - 1; i++) partitions.add(i);

    int currentCost =
        CostAnalyzer.getLatency(Actualizer.actualize(buildQueuesWithPartitions(tasks, partitions)));
    while (true) {
  ...
  public static List<TaskQueue> topoSolve(List<Task> tasks, int[] topoPriority) {
    if (tasks.size() == 0) return new LinkedList<>();
    ...
    PriorityQueue<Task> pq =
        new PriorityQueue<>((a, b) -> topoPriority[a.getID()] - topoPriority[b.getID()]);
    Set<Task> complete = new HashSet<>();

    // add all parent-free nodes to the PQ
    pq.addAll(
        tasks.stream().filter(t -> t.getDependencies().isEmpty()).collect(Collectors.toSet()));
  ...
  public List<TaskQueue> forcedSolve(List<Task> tasks) {
    ...
    for (Task t : tasks) {
      ...
      if (!proposed.getTasks().isEmpty()) {
        ...
        if (!alreadyPresent) graph[last.getID()][i] = t.getID();

        if (checkGraphForCycle()) {
          // this edge creates a cycle!
          // first, remove the edge.
          if (!alreadyPresent) graph[last.getID()][i] = -1;
