...
public class StarsAndBarsNode {
  ...
  private int getLastPartitionPoint() {
    if (partitionPoints.size() == 0) return 0;

    return NullUtils.orThrow(
        partitionPoints.get(partitionPoints.size() - 1),
  ...
  public StarsAndBarsNode getChild(int n) {
    if (n >= getNumChildren()) throw new PermSolveException("Permutation node has no child #" + n);
    ...
    // generate the child
    StarsAndBarsNode toR =
        new StarsAndBarsNode(partitionPoints, n + getLastPartitionPoint() + 1, numItems);
  ...
  /**
   * Gets all the pairs of disconnected items that were created by the addition of the rightmost
   * (highest value) seperator
   *
   * @return the disconnected pairs
   */
  public List<Pair> getAdditionalDisconnectedPairsFromRightmostSeperator() {
    if (partitionPoints.size() <= 2) return getAllDisconnectedPairs();
  ...
  public boolean partitionSizesIncreasing() {
    if (partitionPoints.size() <= 2) return true;

    int lastSize = partitionPoints.get(1) - partitionPoints.get(0);
    for (int i = 2; i < partitionPoints.size(); i++) {
      if (partitionPoints.get(i) - partitionPoints.get(i - 1) < lastSize) return false;
    }
