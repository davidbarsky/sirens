...
public class ScheduleNode {
  ...
  private void calculateLowerBound() {
    if (parentLB == null) {
      ...
      lowerBound = sbn.getAllDisconnectedPairs().stream().mapToInt(p -> costs[p.a][p.b]).sum();
      return;
    }

    // compute the delta with the parent lower bound
    lowerBound =
        sbn.getAdditionalDisconnectedPairsFromRightmostSeperator()
            .stream()
  ...
  public List<TaskQueue> getSchedule() {
    return NullUtils.orThrow(
        Actualizer.actualize(getTaskQueues()),
  ...
  public List<TaskQueue> getTaskQueues() {
    ...
    return NullUtils.orThrow(
        toR, () -> new DAGException("Could not build a schedule using these partitions!"));
  }

  public int getNumChildren() {
    if (isPruned()) return 0; // I've been pruned!
  ...
  public ScheduleNode getChild(int n) {
    if (children.containsKey(n))
      return NullUtils.orThrow(
          children.get(n), () -> new PermSolveException("Child of scheduler node disappeared!"));
  ...
  private Task intToTask(int i) {
    return NullUtils.orThrow(tasks.get(i), () -> new PermSolveException("Null in task list!"));
  }
