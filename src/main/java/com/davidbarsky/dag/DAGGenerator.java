...
public class DAGGenerator {
  ...
  public static Map<String, Map<String, Integer>> getEdgeWeightMap(Collection<Vertex> vertices) {
    ...
    for (Vertex v : vertices) {
      ...
      for (Entry<Vertex, Map<String, String>> e : v.getChildren().entrySet()) {
        edges.put(
            "v" + e.getKey().getID(), Double.valueOf(e.getValue().get("networking")).intValue());
      }
  ...
  public static Collection<TaskQueue> clustersToTasks(
      Collection<Vertex> vertices, Set<List<String>> clusters) {
    Map<String, Task> m =
        verticesToTasks(vertices).stream().collect(Collectors.toMap(v -> "v" + v.getID(), v -> v));
  ...
  public static List<Task> cloneTasks(List<Task> tasks) {
    ...
    List<Task> l = tasks.stream().map(t -> toR.get(t.getID())).collect(Collectors.toList());
