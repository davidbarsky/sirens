...
public class UnboundedCluster {
  ...
  public static List<TaskQueue> generateSchedule(int numNodes) {
    ...
    tasks.forEach(t -> linearClusterDFS(t, new TaskQueue(MachineType.SMALL), paths, visited));

    return paths.stream().distinct().collect(Collectors.toList());
  }

  private static void linearClusterDFS(
      Task task, TaskQueue path, List<TaskQueue> paths, Set<Task> visited) {
    visited.add(task);
    path.add(task);
    paths.add(path);
